...
static void gnss_parser_process_byte(gnss_parser_t *parser, uint8_t byte)
{
    switch (parser->state) {
        ...
        case PARSER_STATE_READ_CRC:
            ...
            if (parser->bytes_read ==
                ...
                if (received_crc == calculated_crc && parser->handler) {
                    const uint8_t *payload_ptr =
                        &parser->msg_buffer[0];
                    parser->handler(parser->msg_id, payload_ptr,
                                    parser->payload_length + GNSS_MESSAGE_OFFSET);
                }
                               ...
                               size_t len)
{
    for (size_t i = 0; i < len; ++i) {
        gnss_parser_process_byte(parser, data[i]);
    }
}
