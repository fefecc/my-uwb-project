...
UWB 协议栈使用主系统传入的当前运行配置，不直接管理 Flash 配置。当前运行配置由主系统配置服务解析：优先使用 Flash 中校验通过的基础配置；如果 Flash 配置不存在或无效，则使用工程内默认静态基础配置。UWB 栈只消费最终生效的 `PAN ID`、本机短地址和角色 `Anchor/Tag`。
...
- 提交待发送原始应用数据给链路层。
- 接收主系统传入的当前运行配置，并把 `PAN ID`、本机短地址、角色等参数下发给链路层。

应用层不直接操作 DW1000，不发送发现帧，不打包空口帧，不修改 MAC/公共头/链路扩展头、`retry_count` 或重传状态。

应用层也不直接读取或写入 Flash。配置是否来自 Flash 或默认静态参数，由主系统配置服务决定；UWB 应用层只使用已经解析完成的配置。
