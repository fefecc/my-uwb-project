...
static uint16_t g_tag_final_delay_ms            = 5;
...
void UWB_SetTagFinalDelayMs(uint16_t delay_ms)
{
    g_tag_final_delay_ms = delay_ms;
}

uint16_t UWB_GetTagFinalDelayMs(void)
{
    return g_tag_final_delay_ms;
}
                    ...
                    if (notified_value & UWB_EVENT_RX_DONE) {
                        ...
                        if (uwb_frame_decode(&resp_frame, dw1000rx_buffer, frame_len) == 0 &&
                            ...
                            uwb_timestamp_t final_tx_ts =
                                uwb_timestamp_add_delay_ms(&g_temp_anchor_record.resp_rx, g_tag_final_delay_ms);
                ...
                if (xTaskNotifyWait(0x00, UINT32_MAX, &notified_value, pdMS_TO_TICKS(100)) == pdTRUE) {
                    if (notified_value & UWB_EVENT_RX_DONE) {
                        ...
                        if (uwb_frame_decode(&result_frame, dw1000rx_buffer, frame_len) == 0 &&
                            ...
                            BaseType_t send_res = xQueueSend(dw1000data_queue, &twr_distance, 0);
                            if (send_res != pdPASS) {
                                log_warn("dw1000 data queue full");
                            }
