 ...
 * @fn dwt_getgpiovalue()
 *
 * @brief This is used to return 1 or 0 depending if the depending if the GPIO
 * is high or low, only one GPIO should be tested at a time
 *
 * input parameters
...
{
    return ((dwt_read32bitoffsetreg(GPIO_CTRL_ID, GPIO_RAW_OFFSET) & gpioNum)
                ? 1
                : 0);
} // change by johhn
/*!
...
void dwt_enablegpioclocks(void)
{
    ...
    dwt_write32bitreg(PMSC_ID,
                      pmsc_clock_ctrl | PMSC_CTRL0_GPCE | PMSC_CTRL0_GPRN);
}
 ...
 * @param txFrameBytes   - Pointer to the user抯 buffer containing the data to
 * send.
 * @param txBufferOffset - This specifies an offset in the DW1000抯 TX Buffer at
...
void dwt_setpanid(uint16 panID)
{
    ...
    dwt_write16bitoffsetreg(PANADR_ID, 2, panID); // PANADR_ID这个是寄存器
}
...
void dwt_otpread(uint32 address, uint32 *array, uint8 length)
{
    ...
    _dwt_enableclocks(
        FORCE_SYS_XTI); // NOTE: Set system clock to XTAL - this is necessary to
                        // make sure the values read by _dwt_otpread are reliable
 ...
 *
 * @brief This is the devices interrupt handler function, it will process/report
 * status events Notes:  In PC based system using (Cheetah or ARM) USB to SPI
 * converter there can be no interrupts, however we still need something to take
 * the place of it and operate in a polled way. In an embedded system this
 * function should be configured to launch on an interrupt, then it will process
 * the interrupt trigger event and call a TX or RX call-back function depending
 * on whether the event is a TX or RX event. The TX call-back will be called
 * when a frame has been sent and the RX call-back when a frame has been
 * received.
 *
 * input parameters
 *
...
void dwt_isr(void) // Assume interrupt can supply context
{
    ...
    //    The status register will have the TXFRS bit set. This function will
    //    clear the tx event and call the dwt_txcallback function.
    //
    // 2. RX Event - if DWT_INT_RFCG is enabled, then when a frame with good CRC
    // has been received the interrupt will be triggered.
    //    The status register will have the RXFCG bit set. This function will
    //    clear the rx event and call the dwt_rxcallback function.
    //
    ...
    //      auto-ACK, the AAT indicates that ACK frame transmission is in
    //      progress. Once the ACK has been sent the TXFRS bit will be set and TX
    //      event triggered. If the auto-ACK is not enabled, the application can
    ...
    if (status & SYS_STATUS_RXFCG) // Receiver FCS Good
    {
                ...
                } else // Double buffer
                {
                         ...
                         << 1)) // Host Side Receive Buffer Pointer
                    {
                        ...
                        dwt_write32bitreg(SYS_STATUS_ID,
                                          clear); // Write status register to clear event
                                                  // bits we have seen
                    }
        ...
        if (status & SYS_STATUS_TXFRS) // Transmit Frame Sent
        {
            ...
            dwt_write32bitreg(
                ...
                clear); // Write status register to clear event bits we have seen
            // NOTE: clear the event which caused interrupt means once the RX is
            // enabled or TX is started New events can trigger and give rise to new
            // interrupts
        ...
        } else if (status & SYS_STATUS_RXRFTO) // Receiver Frame Wait timeout
        {
            ...
            dw1000local.wait4resp = 0;
                   ...
                   SYS_STATUS_ALL_RX_ERR) // Catches all other error events
        {
            ...
            dwt_rxreset(); // Reset the RX
                           // End of fix for bug 622 - LDE done flag gets latched on
                           // a bad frame
